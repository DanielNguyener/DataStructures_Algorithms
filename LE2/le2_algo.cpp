...
int algo3(const vector<int>& lis_arr, int target, int low = 0, int high = -1) {
    if (high == -1) { // if high is default, then it is set to the last index of array
        high = lis_arr.size() - 1;
    }

    if (high < low) { // if the high is less than low (0), it means the range is empty because 0 - 1 = -1.
        return -1;
    }

    int midpoint = (low + high) / 2; //midpoint calculation

    if (lis_arr[midpoint] == target) { // if the target is found at midpoint, return
        return midpoint;
    } else if (target < lis_arr[midpoint]) { // if the target is less than the midpoint
        return algo3(lis_arr, target, low, midpoint - 1); //recursively called, but halves the search range to lower half
    } else {
        return algo3(lis_arr, target, midpoint + 1, high); //recursively called, but halves the searhc range to higher half.
    }
}
